...
//                oWnd.show();
            ...
            <tr>
                <td align="center">
                    <telerik:RadScheduler runat="server" ID="RadScheduler1" SelectedView="TimelineView" Skin="Glow" AllowDelete="false" AllowEdit="false" AllowInsert="false" 
                         DataSourceID="EventsDataSource" SelectedDate="10/21/2016" Height="700px"
                        DataKeyField="ID" DataSubjectField="jobname" DataStartField="JobStartDate" DataEndField="JobEndDate" OnTimeSlotCreated="RadScheduler1_TimeSlotCreated" OnClientAppointmentClick="onAppointmentClick"
                        Localization-HeaderMultiDay="Work Week" OnNavigationComplete="RadScheduler1_NavigationComplete"  > 
                        <AdvancedForm Modal="true"></AdvancedForm> 
                        <ResourceTypes > 
                            <telerik:ResourceType KeyField="ID"  Name="jobname" TextField="jobname" ForeignKeyField="ID"
    ...
    <asp:SqlDataSource ID="EventsDataSource" runat="server" 
        ...
        SelectCommand="SELECT ID, CurveGroupName+' - Click here to view details' as jobname, JobStartDate, JobEndDate FROM [RigTrack].[tblCurveGroup] where isActive=1 and (ID in (select jobid from PrismJobAssignedAssets))"> 
    ...
    <asp:SqlDataSource ID="RoomsDataSource" runat="server" 
        ProviderName="System.Data.SqlClient" ConnectionString="<%$ databaseExpression:client_database %>" 
        SelectCommand="SELECT ID, CurveGroupName as jobname FROM [RigTrack].[tblCurveGroup]  where isActive=1 and (ID in (select jobid from PrismJobAssignedAssets))"> 
    </asp:SqlDataSource> 
