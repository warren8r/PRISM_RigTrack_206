                ...
                <telerik:RadPanelItem runat="server" Expanded="True" 
                    ...
                    <ContentTemplate>
                        <asp:SqlDataSource ID="sqlGetPreVEE" runat="server" 
                            ConnectionString="<%$ ConnectionStrings:elster_database %>" 
                            SelectCommand="SELECT m.MeterName, m.MeterType, m.SerialNumber, ispec.Direction, m.Timezone, m.TimeZoneOffset, ispec.TimestampStart, ispec.TimestampEnd, ispec.Direction AS Expr1, ispec.Interval, r.TimeStamp, r.Value FROM MeterReadings AS mr INNER JOIN Meter AS m ON mr.MeterReadings_Id = m.MeterReadings_Id INNER JOIN IntervalData AS id ON mr.MeterReadings_Id = id.MeterReadings_Id INNER JOIN Reading AS r ON id.IntervalData_Id = r.IntervalData_Id INNER JOIN IntervalSpec AS ispec ON id.IntervalData_Id = ispec.IntervalData_Id WHERE (CONVERT (VARCHAR, ispec.Direction) = 'Delivered') AND (CONVERT (VARCHAR, m.MeterType) = 'REX') AND (ispec.TimestampStart = @TimestampStart) AND (ispec.TimestampEnd = @TimestampEnd)">
                            <SelectParameters>
                                <asp:ControlParameter ControlID="rdStartDt" Name="TimestampStart" 
                                    PropertyName="SelectedDate" />
                                <asp:ControlParameter ControlID="rdStopDt" Name="TimestampEnd" 
                                    PropertyName="SelectedDate" />
                            </SelectParameters>
                        </asp:SqlDataSource>
